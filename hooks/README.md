# MemPalace-rs Hooks — Auto-Save for Terminal AI Tools
...
The AI does the actual filing — it knows the conversation context, so it classifies memories into the right wings, halls, and rooms. The hooks just tell it WHEN to save.
...
The shell scripts are now thin wrappers around the Rust CLI. They resolve the repo root automatically from their own path, so they work regardless of where you install the repo.
