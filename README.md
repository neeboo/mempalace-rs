...
This is the current capability audit against the original MemPalace README and CLI surface, with Python-specific install/runtime details removed:

- [x] `mempalace init` writes room config, detects entities, and generates bootstrap files
- [x] `mempalace mine <dir>` mines project files into the palace
- [x] `mempalace mine <dir> --mode convos` mines conversation exports
- [x] `mempalace mine <dir> --mode convos --extract general` classifies decisions, preferences, milestones, problems, and emotional context
- [x] `mempalace search`, `status`, `wake-up`, `compress`, and `split` are available from the Rust CLI
- [x] AAAK compression, wake-up layers, knowledge graph, palace graph, and MCP server all have Rust implementations
- [x] Claude/Codex auto-save hooks can run through the Rust CLI
- [ ] Interactive `init` entity review flow is not yet as rich as the original Python prompt/edit cycle
- [ ] Specialist-agent manifest/runtime flow from the original README examples is not yet implemented as a Rust feature surface
- [ ] Benchmark runners are still the inherited reference scripts and have not been rewritten as native Rust tooling
...
Either way — your entire memory stack runs offline. SQLite on your machine, Llama on your machine, AAAK for compression, zero cloud calls.
...
let kg = KnowledgeGraph::new(&std::path::PathBuf::from("knowledge_graph.sqlite3"))?;
kg.add_triple("Kai", "works_on", "Orion", Some("2025-06-01"), None, 1.0, None)?;
kg.add_triple("Maya", "assigned_to", "auth-migration", Some("2026-01-15"), None, 1.0, None)?;
kg.add_triple("Maya", "completed", "auth-migration", Some("2026-02-01"), None, 1.0, None)?;

let current = kg.query_entity("Kai", None, QueryDirection::Both)?;
let january = kg.query_entity("Maya", Some("2026-01-20"), QueryDirection::Both)?;
...
```rust
kg.invalidate("Kai", "works_on", "Orion", Some("2026-03-01"))?;
# Ok::<(), Box<dyn std::error::Error>>(())
```
...
Status: planned parity item. Rust diary primitives are implemented, but the manifest-driven `mempalace_list_agents` workflow shown below is not yet part of the Rust CLI.
...
|-----------|------|-------|-----------|
| **LongMemEval R@5** | Raw (SQLite palace only) | **96.6%** | Zero |
...
The Rust CLI now implements `init`, `mine`, `search`, `compress`, `wake-up`, `split`, `status`, and `mcp-server`.

Use `--palace <path>` as a global flag to override the default location:

```bash
mempalace --palace /tmp/palace search "auth decisions"
```

Run the Rust MCP server over stdio with:

```bash
mempalace --palace /tmp/palace mcp-server
```
...
| `src/config.rs` | Rust configuration loading and defaults |
| `src/normalize.rs` | Rust transcript normalization |
| `src/storage.rs` | Rust local storage (SQLite) |
| `src/miner.rs` | Rust project ingest |
| `src/convo.rs` | Rust conversation ingest |
| `src/search.rs` | Rust search path |
| `src/dialect.rs` | Rust AAAK compression |
| `src/entity_detector.rs` | Rust entity detection |
| `src/entity_registry.rs` | Rust entity registry |
| `src/general_extractor.rs` | Rust general memory extraction |
| `src/onboarding.rs` | Rust bootstrap and onboarding helpers |
| `src/hook_protocol.rs` | Rust hook protocol handlers |
| `src/knowledge_graph.rs` | Rust temporal knowledge graph |
| `src/layers.rs` | Rust 4-layer wake-up stack |
| `src/palace_graph.rs` | Rust room graph traversal |
| `src/split.rs` | Rust mega-file splitter |
| `src/mcp_server.rs` | Rust MCP server |
| `mempalace/` | Upstream Python reference kept for comparison during the rewrite |
...
├── Cargo.toml                 ← Rust package + CLI target
├── src/                       ← Rust core path
│   ├── main.rs                ← Rust CLI
│   ├── config.rs              ← Rust config loading
│   ├── normalize.rs           ← Rust transcript normalization
│   ├── storage.rs             ← Rust SQLite storage
│   ├── miner.rs               ← Rust project ingest
│   ├── convo.rs               ← Rust conversation ingest
│   ├── search.rs              ← Rust search path
│   ├── entity_detector.rs     ← Rust entity detection
│   ├── entity_registry.rs     ← Rust entity registry
│   ├── general_extractor.rs   ← Rust general extraction
│   ├── onboarding.rs          ← Rust bootstrap helpers
│   └── hook_protocol.rs       ← Rust hook runtime
├── docs/plans/                ← migration and execution plans
├── mempalace/                 ← upstream Python reference, not the runtime path
...
│   ├── basic_mining.py        ← inherited reference example
│   ├── convo_import.py        ← inherited reference example
│   └── mcp_setup.md
├── tests/                     ← test suite (README)
├── assets/                    ← logo + brand assets
└── pyproject.toml             ← inherited reference packaging metadata
```
...
```bash
cargo test
cargo run --bin mempalace -- init <dir>
```
...
[release-link]: https://github.com/neeboo/mempalace-rs
[license-shield]: https://img.shields.io/badge/license-MIT-b0e8ff?style=flat-square&labelColor=0a0e14
[license-link]: https://github.com/neeboo/mempalace-rs/blob/main/LICENSE
